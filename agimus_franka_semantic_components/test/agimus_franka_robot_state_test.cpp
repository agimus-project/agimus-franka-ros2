...
void AgimusFrankaRobotStateTest::SetUp() {
  ...
  full_interface_names.emplace_back(robot_name + "/" +
                                    agimus_franka_state_interface_name);
  agimus_franka_state_friend =
      std::make_unique<AgimusFrankaRobotStateTestFriend>(
          robot_name + "/" + agimus_franka_state_interface_name);

  std::vector<std::string> interface_names =
      agimus_franka_state_friend->get_state_interface_names();
  ...
  agimus_franka_state_friend->assign_loaned_state_interfaces(
      temp_state_interfaces);
  ASSERT_TRUE(agimus_franka_state_friend->get_values_as_message(
      agimus_franka_robot_state_msg));
}

TEST_F(AgimusFrankaRobotStateTest, validate_state_names_and_size) {
  ASSERT_EQ(agimus_franka_state_friend->name_,
            robot_name + "/" + agimus_franka_state_interface_name);
  ...
  ASSERT_TRUE(std::equal(agimus_franka_state_friend->interface_names_.begin(),
                         agimus_franka_state_friend->interface_names_.end(),
                         full_interface_names.begin(),
...
TEST_F(
  ...
  ASSERT_THAT(joint_angles,
              ::testing::ElementsAreArray(
                  agimus_franka_robot_state_msg.measured_joint_state.position));
  ASSERT_THAT(joint_velocities,
              ::testing::ElementsAreArray(
                  agimus_franka_robot_state_msg.desired_joint_state.position));

  ASSERT_EQ(end_effector_pose[12],
            agimus_franka_robot_state_msg.o_t_ee.pose.position.x);
  ASSERT_EQ(end_effector_pose[13],
            agimus_franka_robot_state_msg.o_t_ee.pose.position.y);
  ASSERT_EQ(end_effector_pose[14],
            agimus_franka_robot_state_msg.o_t_ee.pose.position.z);

  ASSERT_EQ(
      agimus_franka_msgs::msg::AgimusFrankaRobotState::ROBOT_MODE_USER_STOPPED,
...
TEST_F(AgimusFrankaRobotStateTest,
       givenInitializedRobotStateMsg_thenCorrectFrameIDs) {
  agimus_franka_state_friend->initialize_robot_state_msg(
      agimus_franka_robot_state_msg);

  ASSERT_EQ(agimus_franka_robot_state_msg.o_t_ee.header.frame_id, "fr3_link0");
  ASSERT_EQ(agimus_franka_robot_state_msg.ee_t_k.header.frame_id,
            "fr3_hand_tcp");
  ASSERT_EQ(agimus_franka_robot_state_msg.measured_joint_state.name[1],
            "fr3_joint2");
  ASSERT_EQ(agimus_franka_robot_state_msg.k_f_ext_hat_k.header.frame_id,
            "fr3_hand_tcp");
  ASSERT_EQ(agimus_franka_robot_state_msg.o_f_ext_hat_k.header.frame_id,
            "fr3_link0");
  ASSERT_EQ(agimus_franka_robot_state_msg.o_dp_ee_c.header.frame_id,
            "fr3_link0");
  ASSERT_EQ(agimus_franka_robot_state_msg.o_ddp_ee_c.header.frame_id,
            "fr3_link0");
}
