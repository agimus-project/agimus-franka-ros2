...
auto AgimusFrankaRobotState::is_gripper_loaded() -> bool {
...
auto AgimusFrankaRobotState::get_robot_name_from_urdf() -> std::string {
  return model_->name_;
}
...
auto AgimusFrankaRobotState::set_links_from_urdf() -> void {
  auto root_link = model_->getRoot();
  link_names.push_back(root_link->name);
  set_child_links(root_link);
}

auto AgimusFrankaRobotState::set_joints_from_urdf() -> void {
...
auto AgimusFrankaRobotState::get_robot_state() -> agimus_franka::RobotState* {
