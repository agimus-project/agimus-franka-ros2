...
FrankaSemanticComponentInterface::FrankaSemanticComponentInterface(
    const std::string& name, size_t state_interface_size,
...
FrankaSemanticComponentInterface::get_state_interface_names() {
...
std::vector<std::string>
FrankaSemanticComponentInterface::get_command_interface_names() {
  if (command_interface_names_.empty()) {
    for (auto i = 0u; i < command_interface_names_.capacity(); ++i) {
      command_interface_names_.emplace_back(name_ + "/" +
                                            std::to_string(i + 1));
    }
  }
...
std::vector<double>
FrankaSemanticComponentInterface::get_values_state_interfaces() const {
...
std::vector<double>
FrankaSemanticComponentInterface::get_values_command_interfaces() const {
...
bool FrankaSemanticComponentInterface::set_values(
