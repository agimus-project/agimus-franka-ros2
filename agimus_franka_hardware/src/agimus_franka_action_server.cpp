...
#include "agimus_franka_hardware/agimus_franka_action_server.hpp"
...
ActionServer::ActionServer(const rclcpp::NodeOptions& options,
  ...
  error_recovery_action_server_ =
      rclcpp_action::create_server<agimus_franka_msgs::action::ErrorRecovery>(
          this, "~/error_recovery",
          [](auto /*uuid*/, auto /*goal*/) {
          ...
          [this](const auto& goal_handle) {
            errorRecoveryAction(goal_handle);
          });
...
auto ActionServer::errorRecoveryAction(
  ...
  auto result =
      std::make_shared<agimus_franka_msgs::action::ErrorRecovery::Result>();
  try {
    ...
    RCLCPP_ERROR(this->get_logger(),
                 "Network exception thrown automatic error recovery %s",
