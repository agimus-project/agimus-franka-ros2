...
/**
 * This class is a thin wrapper around a @ref agimus_franka::Model and delegates
 * all calls to that
 */
class Model {  // NOLINT(cppcoreguidelines-pro-type-member-init,
 ...
 public:
  /**
   * Create a new Model instance wrapped around a agimus_franka::Model
   */
  explicit Model(agimus_franka::Model* model) : model_(model) {}
   ...
   *
   * @param[in] q Joint position.
   * @param[in] I_total Inertia of the attached total load including end
   * effector, relative to center of mass, given as vectorized 3x3 column-major
   * matrix. Unit: \f$[kg \times m^2]\f$.
  ...
  /**
   * Calculates the Coriolis force vector (state-space equation): \f$ c= C
   * \times dq\f$, in \f$[Nm]\f$.
   *
   * @param[in] q Joint position.
   ...
   * @return Vectorized 4x4 pose matrix, column-major.
   */
  [[nodiscard]] virtual std::array<double, 16> pose(
      agimus_franka::Frame frame,
   ...
   * @return Vectorized 7x7 mass matrix, column-major.
   */
  [[nodiscard]] virtual std::array<double, 49> mass(
      const agimus_franka::RobotState& robot_state) const {
  ...
  /**
   * Calculates the gravity vector. Unit: \f$[Nm]\f$. Assumes default gravity
   * vector of -9.81 m/s^2
   *
   * @param[in] q Joint position.
   * @param[in] m_total Weight of the attached total load including end
   * effector. Unit: \f$[kg]\f$.
