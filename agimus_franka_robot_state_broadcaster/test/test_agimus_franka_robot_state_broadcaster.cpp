...
class MockAgimusFrankaRobotState
    : public agimus_franka_semantic_components::AgimusFrankaRobotState {
 public:
  MockAgimusFrankaRobotState(const std::string& name,
                             const std::string& robot_description)
      : AgimusFrankaRobotState(name, robot_description) {};

  MOCK_METHOD(void, initialize_robot_state_msg,
              (agimus_franka_msgs::msg::AgimusFrankaRobotState&), (override));
};
 ...
 protected:
  void SetUp() override {
    agimus_franka_robot_state_ = std::make_unique<MockAgimusFrankaRobotState>(
        "mock_franka_robot_state",
        ros2_control_test_assets::minimal_robot_urdf);
    broadcaster_ = std::make_unique<AgimusFrankaRobotStateBroadcaster>(
        std::move(agimus_franka_robot_state_));
    ...
    broadcaster_->init("test_broadcaster",
                       ros2_control_test_assets::minimal_robot_urdf, 50.0, "",
                       node_options);
#else
    broadcaster_->init("test_broadcaster");
...
TEST_F(TestAgimusFrankaRobotStateBroadcaster, test_init_return_success) {
  EXPECT_EQ(broadcaster_->on_init(),
...
TEST_F(TestAgimusFrankaRobotStateBroadcaster,
       test_update_without_franka_state_interface_returns_error) {
  EXPECT_EQ(broadcaster_->on_configure(rclcpp_lifecycle::State()),
            controller_interface::CallbackReturn::SUCCESS);
  EXPECT_EQ(broadcaster_->on_activate(rclcpp_lifecycle::State()),
            controller_interface::CallbackReturn::SUCCESS);
  ...
  EXPECT_EQ(broadcaster_->update(time, period),
            controller_interface::return_type::ERROR);
}

TEST_F(TestAgimusFrankaRobotStateBroadcaster,
       test_update_with_franka_state_returns_success) {
  // Todo(anyone)
