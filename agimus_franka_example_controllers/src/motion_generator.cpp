...
#include <Eigen/Core>
#include <agimus_franka_example_controllers/motion_generator.hpp>
#include <algorithm>
#include <array>
#include <cassert>
#include <cmath>
...
bool MotionGenerator::calculateDesiredValues(double time,
        ...
        (*delta_q_d)[i] =
            ...
            0.5 *
                (1.0 / std::pow(delta_t_2_sync[i], 3.0) *
                 ...
                 (2.0 * time - 2.0 * t_1_sync_[i] - delta_t_2_sync[i] -
                  2.0 * t_d[i])) *
...
std::pair<MotionGenerator::Vector7d, bool>
MotionGenerator::getDesiredJointPositions(
  ...
  Eigen::VectorXd::Map(joint_positions.data(), kJoints) =
      (q_start_ + delta_q_d);
