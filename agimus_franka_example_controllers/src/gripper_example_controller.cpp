...
#include "agimus_franka_example_controllers/gripper_example_controller.hpp"
...
CallbackReturn GripperExampleController::on_activate(
    const rclcpp_lifecycle::State&) {
  if (!gripper_move_action_client_->wait_for_action_server(
          std::chrono::seconds(5))) {
    RCLCPP_ERROR(get_node()->get_logger(),
                 "Move Action server not available after waiting.");
    return CallbackReturn::ERROR;
  }
  if (!gripper_grasp_action_client_->wait_for_action_server(
          std::chrono::seconds(5))) {
    RCLCPP_ERROR(get_node()->get_logger(),
                 "Grasp Action server not available after waiting.");
    return CallbackReturn::ERROR;
  }
...
controller_interface::CallbackReturn GripperExampleController::on_deactivate(
  ...
  if (gripper_stop_client_->service_is_ready()) {
    ...
    RCLCPP_ERROR(get_node()->get_logger(),
                 "Gripper stop service is not available.");
  }
  return CallbackReturn::SUCCESS;
}

controller_interface::return_type GripperExampleController::update(
...
void GripperExampleController::assignMoveGoalOptionsCallbacks() {
  move_goal_options_.goal_response_callback =
      [this](const std::shared_ptr<rclcpp_action::ClientGoalHandle<
                 agimus_franka_msgs::action::Move>>& goal_handle) {
        if (!goal_handle) {
          RCLCPP_ERROR(get_node()->get_logger(),
  ...
  move_goal_options_.feedback_callback =
      [this](const std::shared_ptr<rclcpp_action::ClientGoalHandle<
                 agimus_franka_msgs::action::Move>>&,
             const std::shared_ptr<
                 const agimus_franka_msgs::action::Move_Feedback>& feedback) {
...
void GripperExampleController::assignGraspGoalOptionsCallbacks() {
  grasp_goal_options_.goal_response_callback =
      [this](const std::shared_ptr<rclcpp_action::ClientGoalHandle<
                 agimus_franka_msgs::action::Grasp>>& goal_handle) {
        if (!goal_handle) {
          RCLCPP_ERROR(get_node()->get_logger(),
                       RED "Grasp Goal NOT accepted." RESET);
        } else {
  ...
  grasp_goal_options_.feedback_callback =
      [this](const std::shared_ptr<rclcpp_action::ClientGoalHandle<
                 agimus_franka_msgs::action::Grasp>>&,
             const std::shared_ptr<
                 const agimus_franka_msgs::action::Grasp_Feedback>& feedback) {
  ...
  grasp_goal_options_.result_callback =
      [this](const rclcpp_action::ClientGoalHandle<
             agimus_franka_msgs::action::Grasp>::WrappedResult& result) {
...
bool GripperExampleController::openGripper() {
  RCLCPP_INFO(get_node()->get_logger(),
              "Opening the gripper - Submitting a Move Goal");
  ...
  std::shared_future<std::shared_ptr<
      rclcpp_action::ClientGoalHandle<agimus_franka_msgs::action::Move>>>
      move_goal_handle = gripper_move_action_client_->async_send_goal(
          move_goal, move_goal_options_);
  bool ret = move_goal_handle.valid();
  if (ret) {
    RCLCPP_INFO(get_node()->get_logger(), "Submited a Move Goal");
  } else {
    RCLCPP_ERROR(get_node()->get_logger(),
                 RED "Failed to submit a Move Goal" RESET);
  }
  return ret;
}
...
void GripperExampleController::graspGripper() {
  RCLCPP_INFO(get_node()->get_logger(),
              "Closing the gripper - Submitting a Grasp Goal");
  ...
  std::shared_future<std::shared_ptr<
      rclcpp_action::ClientGoalHandle<agimus_franka_msgs::action::Grasp>>>
      grasp_goal_handle = gripper_grasp_action_client_->async_send_goal(
          grasp_goal, grasp_goal_options_);
  ...
  if (ret) {
    ...
    RCLCPP_ERROR(get_node()->get_logger(),
                 RED "Failed to submit a Grasp Goal" RESET);
  }
}
...
PLUGINLIB_EXPORT_CLASS(
    agimus_franka_example_controllers::GripperExampleController,
