...
def test_load_without_gripper():
    urdf = xacro.process_file(fr3_xacro_file_name, mappings={"hand": "false"}).toxml()
    assert urdf.find("fr3_rightfinger") == -1


def test_load_with_arm_id():
    urdf = xacro.process_file(
        fr3_xacro_file_name, mappings={"arm_id": "totally_different_arm"}
    ).toxml()
